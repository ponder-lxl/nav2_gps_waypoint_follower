...
class GpsGuiLogger(tk.Tk, Node):
    ...
    def gps_callback(self, msg: NavSatFix):
        ...
        """
        self.last_gps_position = msg # 保存最新gps消息
        self.updateTextBox() # 更新GUI显示

    def imu_callback(self, msg: Imu):
        """
        Callback to store the last heading
        """
        _, _, self.last_heading = euler_from_quaternion(msg.orientation) # 保存最新IMU的yaw角
        self.updateTextBox()
...
def main(args=None):
    ...
    while rclpy.ok():
        ...
        rclpy.spin_once(gps_gui_logger, timeout_sec=0.1)  # 处理回调函数
        gps_gui_logger.update()  # tkinter处理GUI
